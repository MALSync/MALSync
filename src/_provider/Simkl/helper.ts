...
export async function syncList(lazy = false){
  ...
  var activity = await this.call('https://api.simkl.com/sync/activities');
  con.log('Activity', lastCheck, activity.anime);

  //removed_from_list
  if(lastCheck && (lastCheck.removed_from_list !== activity.anime.removed_from_list)){
    var checkRemoveList = await this.call('https://api.simkl.com/sync/all-items/anime');
    var newCacheList = {};
    if(checkRemoveList){
  ...
  if(lastCheck && (lastCheck.rated_at !== activity.anime.rated_at)){
    var rated = await this.call('https://api.simkl.com/sync/ratings/anime?'+dateFrom);
  ...
  var list = await this.call('https://api.simkl.com/sync/all-items/anime?'+dateFrom);
