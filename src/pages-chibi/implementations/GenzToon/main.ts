...
export const GenzToon: PageInterface = {
  name: 'GenzToon',
  domain: 'https://genzupdates.com/',
  languages: ['English'],
  type: 'manga',
  urls: {
    match: ['*://genzupdates.com/*'],
  },
  search: 'https://genzupdates.com/series/?q={searchtermPlus}',
  sync: {
    isSyncPage($c) {
      return $c
  ...
  overview: {
    ...
    getTitle($c) {
      return $c.querySelector('h1.text-xl').text().trim().run();
    },
    getIdentifier($c) {
    ...
    uiInjection($c) {
      return $c.querySelector('h1.text-xl').uiAfter().run();
    },
  },
  list: {
    elementsSelector($c) {
