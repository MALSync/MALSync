      ...
      >
        ...
        <div class="mdl-card__supporting-text" style="color: black">
          ...
          <div v-if="review.rReadmore">
            ...
            >
            <p
              v-for="(line, index3) in review.rReadmore"
              v-show="activeReadMores.includes(index)"
              :key="index3"
            >
              {{ line }}
    ...
    reviews() {
      ...
      try {
        ...
        j.$.each(j.$(reviewsData2).filter('.borderDark'), (index, value) => {
          ...
          const userImage = imageBlock.find('a > img').first().attr('data-src');

          const username = j.$(value).find('.spaceit > .mb8').next().find('td > a').text().trim();

          const rPeople = Number(
            j.$(value).find('.spaceit > .mb8').next().find('td > div > strong > span').text(),
          );

          const rDate = j.$(value).find('.spaceit > .mb8 > div').first().text().trim();

          const rEpisodes = j.$(value).find('.spaceit > .mb8 > div.lightLink').text().trim();

          const rRating = Number(
            j.$(value).find('.spaceit > .mb8 > div').last().text().replace(/\D+/, ''),
          );
