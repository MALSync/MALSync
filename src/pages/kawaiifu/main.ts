  ...
  sync: {
    ...
    getEpisode(url) {
      if (
        ...
        j.$('ul.list-ep a.active').text().toLowerCase().indexOf('teaser') !== -1
      ) {
        return 0;
      }
    ...
    nextEpUrl(url) {
      const href = j.$('div#server_ep a.active').closest('li').next().find('a').attr('href');
    ...
    uiSelector(selector) {
      j.$('div.desc-top').first().after(j.html(selector));
    },
  },
  overview: {
    getTitle(url) {
