...
export const OtakuFR: pageInterface = {
  ...
  overview:{
    getTitle: function(url){return utils.getBaseText($('h1.ttl'))},
    getIdentifier: function(url){return OtakuFR.sync.getIdentifier(url)},
    uiSelector: function(selector){selector.insertAfter(j.$("#sct_content > div.wpa_pag.anm_det > h1").first());},
    list:{
      offsetHandler: true,
      ...
      elementUrl: function(selector){
        return utils.absoluteLink(selector.find('a').first().attr('href'),OtakuFR.domain);
      },
      elementEp: function(selector){
          var url = OtakuFR.overview!.list!.elementUrl(selector);
          return episodeHelper(urlHandling(url), selector.find('a').text().trim());
      }
    }
    },
...
function urlHandling(url){
  var langslug = j.$('#sct_banner_head > div > a').first().attr('href');
  if(langslug == '/'){
    return url;
  }else{
