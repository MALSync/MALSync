...
export const Isekaiscan: pageInterface = getInter();
  ...
  thisSelf = {
    ...
    overview: {
      ...
      list: {
        ...
        elementEp(selector) {
          return thisSelf.sync.getEpisode(thisSelf.overview!.list!.elementUrl!(selector));
        },
      },
    },
    init(page) {
      ...
      j.$(() => {
        if (thisSelf.isSyncPage(page.url)) {
          page.handlePage();
        }
        if (thisSelf.isOverviewPage!(page.url)) {
          utils.waitUntilTrue(
            function () {
