...
export const PrismaScans: pageInterface = {
  name: 'PrismaScans',
  domain: ['https://prismascans.net'],
  ...
  sync: {
    ...
    nextEpUrl(url) {
      return utils.absoluteLink(j.$('a:contains(Next)').attr('href'), PrismaScans.domain);
    },
  },
  overview: {
    getTitle(url) {
    ...
    list: {
      ...
      elementEp(selector) {
        return PrismaScans.sync.getEpisode(String(selector.find('a').first().attr('href')));
      },
    },
  },
  init(page) {
