...
export class SyncPage {
    constructor(url, pages, floatClick = () => {
        ...
        this.novel = false;
        this.strongVolumes = false;
        this.videoSyncOffset = true;
        this.autoNextEpRun = false;
        ...
        if (!(typeof api.settings.get('enablePages')[this.page.name] === 'undefined' ||
    ...
    handleVideoResume(item, timeCb) {
        if (typeof this.curState === 'undefined' ||
        ...
        if (typeof this.curState.videoChecked !== 'undefined' && this.curState.videoChecked) {
            ...
            if (localItem !== null &&
                ...
                if (!j.$('#MALSyncResume').length)
                    j.$('#MALSyncResume').parent().parent().remove();
    ...
    handlePage(curUrl = window.location.href) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.page.isSyncPage(this.url)) {
                ...
                this.setSearchObj(new SearchClass(state.title, this.novel ? 'novel' : this.page.type, state.identifier));
                ...
                this.curState = state;
                yield this.searchObj.search();
                try {
                    tempSingle = yield this.searchObj.initRules();
                }
                catch (e) {
            ...
            if (malUrl === null) {
                ...
                // sync
                if (this.page.isSyncPage(this.url)) {
                    const rerun = yield this.searchObj.openCorrectionCheck();
                    if (rerun) {
                        // If malUrl changed
                    ...
                    if (yield this.singleObj.checkSync(state.episode, state.volume)) {
                        if (!(this.strongVolumes && !state.episode))
                        ...
                        yield this.startSyncHandling(state, malUrl);
                    }
                    else {
    ...
    startSyncHandling(state, malUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            let mangaProgressMode = false;
            if (this.mangaProgress) {
                try {
                    mangaProgressMode = yield this.mangaProgress.start();
            ...
            if (!mangaProgressMode && api.settings.get(`autoTrackingMode${this.page.type}`) === 'instant') {
                ...
                let message = `<button class="sync" style="margin-bottom: 8px; background-color: transparent; border: none; color: rgb(255,64,129);margin-top: 10px;cursor: pointer;">${api.storage.lang(translationMsg.key, [providerTemplates(malUrl).shortName, translationMsg.value])}</button>`;
                let options = {
                    hoverInfo: true,
                    error: true,
                    type: 'update',
                ...
                if (api.settings.get(`autoTrackingMode${this.page.type}`) === 'video' &&
        ...
        ${message}`;
                ...
                if (this.singleObj.getType() === 'anime') {
                    playerTimeout = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                        j.$('#flashinfo-div').addClass('player-error');
                        if (yield hasMissingPermissions()) {
    ...
    syncHandling(hoverInfo = false, undo = false) {
        let p;
        if (undo) {
        ...
        return p
            ...
            if (typeof diffState.onList === 'undefined')
                diffState.onList = true;
    ...
    fillUI() {
        ...
        if (this.singleObj.getLastError()) {
            ...
            j.$('#MalInfo').text('');
            j.$('#malRating').after(j.html(`&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id='LoginMalDiv'>${this.singleObj.getLastErrorMessage()}</span>`));
            return;
        }
        let scoreCheckbox = '';
        ...
        if (!this.singleObj.isOnList()) {
            j.$('.MalLogin').css('display', 'none');
            j.$('#malRating').after(j.html(`<span id='AddMalDiv'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='#' id='AddMal' onclick='return false;'>${api.storage.lang(`syncPage_malObj_addAnime`, [this.singleObj.shortName])}</a></span>`));
            const This = this;
        ...
        if (typeof this.page.overview !== 'undefined' &&
            ...
            this.page.overview.list.elementsSelector().each(function (index, el) {
                try {
                    ...
                    if ((api.settings.get('highlightAllEp') && elEp <= currentEpisode) ||
                        elEp === currentEpisode) {
    ...
    offsetHandler(epList) {
        if (!this.page.overview.list.offsetHandler)
            return;
        if (this.getOffset())
            return;
        if (!this.searchObj || this.searchObj.provider === 'user')
            return;
    ...
    loadUI() {
        ...
        ui += wrapEnd;
        // ui += '<span id="MalLogin">';
        const wrapStart = (section) => `<span style="display: inline-block; display: none;" class="MalLogin malp-group malp-group-${section}">`;
        ui += wrapStart('status');
            ...
            if (info.action === 'presence') {
                ...
                if (this.curState) {
                    ...
                    if (api.settings.get('presenceShowButtons')) {
                        let url = null;
                        if (this.singleObj.shortName !== 'Local')
                            url = this.singleObj.getDisplayUrl();
                        if (!url)
                            url = this.singleObj.getMalUrl();
                        if (!url && !presenceHidePage)
                            url = this.singleObj.getStreamingUrl();
                        if (url) {
    ...
    checkForFiller(malid, episode) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(yield cacheObj.hasValueAndIsNotEmpty())) {
                ...
                const request = yield api.request.xhr('GET', url).then((response) => __awaiter(this, void 0, void 0, function* () {
                    try {
