...
async function nineanime() {
  ...
  const urls = $('ul > li > a[target="_blank"]:not(:contains(alternative)):not(:contains(lite))')
...
async function gogoanime() {
  ...
  const urls = $('.note_site .site_go a')
    .map((i, el) => new URL($(el).attr('href')))
    .get();
  ...
  for (let url of urls) {
    ...
    addpageUrls('Gogoanime', ['*://*.' + url.hostname + '/*']);
  }
}
...
async function kickassanime() {
  ...
  const urls = $('div.container a.button')
    .map((i, el) => new URL($(el).attr('href')))
    .get();
...
async function anix() {
  ...
  const urls = $('.mt-3 a[target="_blank"]')
    .map((i, el) => new URL($(el).attr('href')))
    .get();

  for (let url of urls) {
    addpageUrls('Anix', ['*://*.' + url.hostname + '/anime/*']);
  }
}
